...
case class ColourGradient(name: String,
  ...
  def toSvgDefs(size: Double = 50): Tag =
    linearGradient(id := name, x1 := -size / 2, y1 := -size / 2, x2 := size / 2, y2 := -size / 2)(
  ...
  def toSvg(size: Double = 50,
    ...
    val textTag: Tag = text(x := size / 2, y := size / 2 + textSize / 2 - textAdjust,
      fill := s"#$textHex", textAnchor := "middle", fontFamily := titleFont, fontWeight := "bold", fontSize := textSize)
    g(
      circle(cx := 0, 0, r := size / 2, fill := s"url(#$name)",

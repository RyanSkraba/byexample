...
/**
  ...
  * @param stepX       Number of pixels to move in the x direction for a step.
  * @param stepY       Number of pixels to move in the y direction for a step.
  * @param penDown     True to leave a trail as the stamper is moved.
  * @param tag         The SVG tag to reuse when moving.
...
case class Stamper(
  ...
  /**
    ...
    * @param tag The tag to use from now on (unchanged by default)
  ...
  /**
    ...
    * @param withTag A function mapping a character at a position in the string to a tag.
    */
  def draw(
      pattern: String,
  ...
  /**
    ...
    * the [[stepX]] and [[stepY]] parameters will be increased by the relative locations.
    *
    * @param id The SVG element id to use for the original history.
    * @param dx Where to position the clone horizontally.
    * @param dy Where to position the clone vertically.
    */
  def clone(
